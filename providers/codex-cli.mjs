...
export function buildCodexPrompt(prompt, options = {}) {
  ...
  return [
    ...
    "This turn runs through the OpenAI Codex CLI on the organism's required model assignment.",
    ...
    `ATLAS mode: ${mode}.`,
    "The Claude Agent SDK in-process fleet MCP tools are not attached to this external process. Deterministic Hermes organs are attached through the repository's validated Atlas tool bridge.",
    "Attached bridge tools: abolish_work, compile_obligations, inspect_source, and economic_radar. Invoke abolition with `node scripts/atlas-tool.cjs abolish_work --limit=3 --dry-run`; invoke the live SEC obligation proof with `node scripts/atlas-tool.cjs compile_obligations --sec-mesa`, or compile operator-supplied JSON with `--input=<json-path>`. If semantic source search is unavailable, use `node scripts/atlas-tool.cjs inspect_source --query=<text> --path=<scope> --limit=20`; it is bounded, read-only, and source-hashed. For a deterministic, read-only public code-bounty scan use `node scripts/atlas-tool.cjs economic_radar --min-usd=100 --limit=20`; its packet grants no authority, advertised dollars are not revenue, and every outward action requires Daniel's fresh approval. The SEC proof is read-only and must not describe exchange value as savings.",
    "Do not claim an MCP call or claim other fleet tools such as spawn_agent, verify_build, or shard_memory unless a real callable route is present.",
